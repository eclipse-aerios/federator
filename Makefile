...
# =============================================================================
# Project Settings
PROJECT_NAME ?= federator
ORGANIZATION ?= aerios-project.eu
DESCRIPTION ?= "aeriOS Federator"
MAINTAINER ?= "ravagar2@upv.es"
...
docker-buildx-gitlab: 
	...
	- $(CONTAINER_TOOL) buildx build --provenance=false --build-arg COMMIT_HASH=$(git rev-parse HEAD) --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile .
